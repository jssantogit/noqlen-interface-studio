...
| Library folder selection | "Choose folder" button then folder option card tap in sheet | Selected folder card appears on setup step | `libraryPath` and `navidromeDraft.MusicFolder` update | Static fake folder options | Filesystem access, file picker | implemented |
| Setup folder picker sheet | "Choose folder" button | Bottom sheet with selectable folder rows and custom path input | Local picker open state | Static fake folder options | Filesystem access, file picker | implemented |
| Custom mock path entry | "Use another folder..." in picker | Inline input for mock path with visual validation | Custom path saved to `libraryPath` | User-typed mock string | Filesystem access, path validation | implemented |
| Continue without folder | Continue tap when no folder selected | Disabled button with helper text | None | Setup draft state | None | implemented |
...
| Advanced Navidrome settings from setup | `Advanced Navidrome Settings` row | Existing Navidrome Settings sheet opens inside setup | Active sheet becomes `navidromeSettings`; setup draft updates for basic fields | Existing Navidrome Settings component | Duplicate settings or real config access | implemented |
| Setup/Navidrome draft sync | Editing basic fields in Advanced Settings | Updated values reflect in setup Navidrome Basics step | `navidromeDraft` updates via `onDraftChange` bridge | Setup draft + catalog draft | Real config access | partial |
