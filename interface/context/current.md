...
- Forge Enrich Mode is implemented as a full-screen 6-step force rewrite flow (Rewrite options → Target selection → Confirmation → Dry-run → Progress → Result) accessible from Review / All. Step 1 was refined with collapsible category cards, count-based selection badges, improved caution copy, a compact Settings helper row, sticky header and no automatic entry toast.
