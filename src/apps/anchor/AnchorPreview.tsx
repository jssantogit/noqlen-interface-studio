...
export function AnchorPreview() {
  return (
    <div className="min-h-full bg-[radial-gradient(circle_at_20%_0%,rgba(245,158,11,0.22),transparent_18rem),linear-gradient(180deg,#111827_0%,#090d12_100%)] px-5 py-5 text-white">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <article className="rounded-[1.5rem] border border-white/10 bg-black/24 p-4">
          <Folder className="text-amber-100" size={22} />
          <p className="mt-4 text-sm text-slate-400">Library</p>
          <p className="mt-1 text-xl font-semibold">Curated view</p>
        </article>
        <article className="rounded-[1.5rem] border border-white/10 bg-black/24 p-4">
          <Database className="text-amber-100" size={22} />
          <p className="mt-4 text-sm text-slate-400">Index</p>
          <p className="mt-1 text-xl font-semibold">Static state</p>
        </article>
      </div>
      ...
      <section className="mt-4 rounded-[1.5rem] border border-white/10 bg-white/[0.045] p-4">
        ...
        <div className="mt-3 flex gap-2 overflow-hidden">
          {['Scan', 'Queue', 'Review'].map((item) => (
            <span
              className="rounded-full border border-amber-200/15 bg-amber-200/[0.07] px-4 py-2 text-xs text-amber-50/80"
              key={item}
            >
              {item}
            </span>
