...
export function AnchorNavidromeDryRunSheet({
  ...
  return (
    <div className="absolute inset-0 z-50 flex min-w-0 items-end overflow-hidden bg-black/56 px-2 pt-8 backdrop-blur-[2px]">
      <button aria-label="Close dry-run backdrop" className="absolute inset-0 cursor-default" onClick={onClose} type="button" />
      <section className="relative max-h-[82%] w-full min-w-0 overflow-hidden rounded-t-[1.55rem] border border-white/[0.09] bg-[linear-gradient(180deg,rgba(20,32,38,0.99),rgba(6,12,17,0.99))] shadow-[0_-1.2rem_3rem_rgba(0,0,0,0.5)]">
        ...
        <div className="max-h-[calc(82vh-6rem)] min-w-0 space-y-3 overflow-y-auto overflow-x-hidden px-5 pb-5">
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.04] p-3">
              ...
              <div className="text-[0.62rem] uppercase tracking-[0.14em] text-slate-400">changed</div>
            </div>
            <div className="rounded-2xl border border-amber-300/14 bg-amber-300/[0.055] p-3">
              <div className="text-lg font-semibold text-amber-100">{restartCount}</div>
              <div className="text-[0.62rem] uppercase tracking-[0.14em] text-amber-200/74">restart</div>
            </div>
            <div className="rounded-2xl border border-orange-300/14 bg-orange-300/[0.055] p-3">
              <div className="text-lg font-semibold text-orange-100">{warningCount}</div>
              <div className="text-[0.62rem] uppercase tracking-[0.14em] text-orange-200/74">warnings</div>
            </div>
          </div>
            ...
            {changed.length === 0 ? (
              ...
              <div className="min-w-0 rounded-2xl border border-white/[0.065] bg-white/[0.04] p-3" key={option.key}>
                <div className="flex min-w-0 items-center justify-between gap-2">
                  <h3 className="truncate text-xs font-semibold text-white">{option.key}</h3>
          ...
          <pre className="max-h-44 min-w-0 overflow-auto whitespace-pre-wrap break-words rounded-2xl border border-white/[0.065] bg-black/30 p-3 text-[0.66rem] leading-4 text-slate-200/86">
          ...
          <div className="grid grid-cols-2 gap-2.5 pt-1">
            <button className="h-10 rounded-xl border border-white/[0.075] bg-white/[0.045] text-sm font-medium text-white" onClick={onClose} type="button">Close</button>
            <button className="h-10 rounded-xl bg-amber-400 text-sm font-semibold text-[#211508] shadow-[0_0.8rem_1.5rem_rgba(245,158,11,0.16)]" onClick={onApply} type="button">Apply mock changes</button>
          </div>
        </div>
      </section>
    </div>
