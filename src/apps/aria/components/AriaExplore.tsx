        ...
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              onClick={() => onOpenExploreMode(cat.id as ExploreMode, cat.label)}
              type="button"
            >
      ...
      <section className="mt-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            onClick={() => onOpenExploreMode('recent', 'Recently explored')}
            type="button"
          >
            See all
          </button>
        ...
        <div className="mt-2.5 grid grid-cols-4 gap-2">
          {recentlyExplored.map((item) => (
            <button
              ...
              key={item.id}
              onClick={() => {
                if (item.label === 'Artist') onOpenExploreMode('artists', 'Artists')
                else if (item.label === 'Albums') onOpenExploreMode('albums', 'Albums')
                else if (item.label === 'Genres') onOpenExploreMode('genres', 'Genres')
                else onOpenExploreMode('recent', 'Recently Added')
              }}
              type="button"
            >
