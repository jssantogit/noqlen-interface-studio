...
export function AriaLibrarySongsView({
  onBack,
  onOpenTrack,
  onShowToast,
  tracks,
    ...
    <div className="min-h-full min-w-0 overflow-x-hidden px-4 pt-4 text-[#f5ecdf]">
      ...
      <header className="mt-4 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="font-serif text-[42px] leading-[0.95] text-[#fff3e4]">Songs</h1>
          <p className="mt-1.5 text-[16px] text-[#b9b1a7]">1,200 local tracks</p>
        </div>
        <HeaderTools label="songs" onShowToast={onShowToast} />
      </header>

      <section className="mt-5 overflow-hidden rounded-[20px] border border-white/[0.075] bg-white/[0.028] p-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
        {tracks.map((track, index) => (
          <button className="group flex w-full min-w-0 items-center gap-3 rounded-[15px] px-2.5 py-2 text-left transition hover:bg-white/[0.045]" key={`${track.id}-${index}`} onClick={() => onOpenTrack(track)} type="button">
            <span className={`h-9 w-9 shrink-0 rounded-[11px] bg-gradient-to-br ${track.accent} shadow-[0_8px_18px_rgba(0,0,0,0.24)]`} />
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[15px] font-semibold leading-tight text-[#fff3e4]">{track.title}</span>
              <span className="mt-0.5 block truncate text-[12px] text-[#b9b1a7]">{track.artist} · {track.album}</span>
            </span>
            <span className="shrink-0 text-[12px] tabular-nums text-[#a79d91]">{track.duration}</span>
          </button>
        ))}
      </section>
