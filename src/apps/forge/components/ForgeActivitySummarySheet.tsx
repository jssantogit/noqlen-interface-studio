...
export function ForgeActivitySummarySheet({
  item,
  onClose,
  onOpenRelatedReview,
}: {
        ...
        <div className="rounded-xl bg-white/[0.04] p-4">
          ...
          <p className="mt-1 text-[15px] font-semibold text-white">
            {item.affectedCount > 0
              ? `${item.affectedCount} item${item.affectedCount === 1 ? '' : 's'} ${item.activityType === 'libraryCheck' ? 'checked' : 'updated'}`
              : item.subtitle}
          </p>
          {item.changedFields && item.changedFields.length > 0 ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {item.changedFields.map((field) => (
                <span
                  key={field}
                  className="rounded-lg bg-white/[0.06] px-2 py-1 text-xs text-white/72"
                >
                  {field}
                </span>
              ))}
        ...
        <div className="flex flex-col gap-2.5 pt-1">
          {onOpenRelatedReview ? (
            <button
              ...
              type="button"
            >
              Open related review <ArrowRight size={15} />
            </button>
          ) : null}
          <button
